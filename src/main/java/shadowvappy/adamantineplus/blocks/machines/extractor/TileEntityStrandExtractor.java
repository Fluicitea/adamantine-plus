package shadowvappy.adamantineplus.blocks.machines.extractor;
...
public class TileEntityStrandExtractor extends TileEntity implements ITickable {
    ...
    public ITextComponent getDisplayName() {
        return new TextComponentTranslation("container.strand_extractor");
    }
    ...
    public void setState(boolean active, World world, BlockPos pos) {
        ...
        if(active != state.getValue(BlockStrandExtractor.BURNING)) {
            if(active)
                world.setBlockState(pos, state.withProperty(BlockStrandExtractor.BURNING, true));
            else
                world.setBlockState(pos, state.withProperty(BlockStrandExtractor.BURNING, false));
        }
    ...
    public void update() {
        ...
        if(this.isBurning() || !fuel.isEmpty()) {
            ...
            if(!this.handler.getStackInSlot(0).isEmpty() || this.getCookTime() > 0) { //If input slot is not empty or is extracting
                if(!this.isBurning() && (this.canSmelt() || this.getCookTime() > 0)) { //If the strand extractor isn't burning and can smelt or is smelting
            ...
            if(this.getResult() != ItemStack.EMPTY && this.getCookTime() > 0) {
                ...
                if(this.getCookTime() == this.getTotalCookTime()) { //If cook time is total cook time
                    if(this.handler.getStackInSlot(2).getCount() > 0 && //If is one or more items in the output slot...
                        ...
                        this.handler.getStackInSlot(2).grow(this.result.getCount()); //Increase the stack in the output by the count of the result
                    else this.handler.insertItem(2, this.result, false); //Else insert the recipe result into the output slot
                ...
                int inputCount = StrandExtractorRecipes.getInstance().getInputCount(input); //Get the input stack's item count
                ItemStack result = StrandExtractorRecipes.getInstance().getExtractingResult(input).copy(); //Get the result of the recipe
    ...
    public static boolean isBurning(TileEntityStrandExtractor te) {
    ...
    public int getCookTime(ItemStack input) {
        return StrandExtractorRecipes.getInstance().getExtractingTime(input);
    }
    ...
    private boolean canSmelt() {
        if(this.handler.getStackInSlot(0).isEmpty())
            ...
            ItemStack result = StrandExtractorRecipes.getInstance().getExtractingResult(this.handler.getStackInSlot(0)).copy();
            if(result.isEmpty())
                return false;
            else {
